...
import { AuthSigninModel } from './../auth/auth.signin.model';
...
export class AuthService {
  ...
  userInfo   = new Subject<UserModule>();
  ...
  login(authSModel: AuthSigninModel) {
    this.http.post<{token: string, message: string, expiresIn: number, authId: string, roll: string}>(
      BACKEND_URL  + '/auth/signin', authSModel
      ).subscribe(
          result => {
            ...
            if (token && roll === 'root') {
              ...
              this.router.navigate(['/dashbord/pages/home']);
            }
          },
          err => {
